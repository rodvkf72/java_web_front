      ...
      <div class="col-lg-8 col-md-10 mx-auto">
          ...
          {:else}
            ...
            {#each resultList as item}
              <tbody style="text-align: center;">
                ...
                <td><hr>&nbsp;<br><a href="/board/{divi}/view/{item.pk}">{item.title}</a><br>&nbsp;</td>
              </tbody>
            {/each}
          {/if}
        ...
        <div class="clearfix page_wrap">
          <div id="b_dv" class="page_nation" style="text-align: center">
            ...
            <a class="arrow pprev" href="#" onclick="location.href='/board/{divi}/1'"></a>
            {#if page <= 1}
                <a class="arrow prev" href="#" onclick="location.href='/board/{divi}/1'"></a>
            {:else}
                <a class="arrow prev" href="#" onclick="location.href='/board/{divi}/{Number(page)-Number(1)}'"></a>
            {/if}

            {#each paging as item}
                {#if page == item.no}
                        <a class="active" href="#" onclick="location.href='/board/{divi}/{item.no}'">{item.no}</a>
                {:else}
                        <a href="#" onclick="location.href='/board/{divi}/{item.no}'">{item.no}</a>
                {/if}
<!--
            ...
            {#if page >= Math.ceil(max / 10)}
                <a class="arrow next" href="#" onclick="location.href='/board/{divi}/{Math.ceil(max / 10)}'"></a>
            {:else}
                <a class="arrow next" href="#" onclick="location.href='/board/{divi}/{Number(page)+Number(1)}'"></a>
            {/if}
            <a class="arrow nnext" href="#" onclick="location.href='/board/{divi}/{Math.ceil(max / 10)}'"></a>
